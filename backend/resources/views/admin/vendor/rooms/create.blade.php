...
@section('title', __('admin.vendor.rooms.add'))
@section('content')
<div class="container-fluid">
    <x-page-title title="{{ __('admin.vendor.rooms.add') }}" :breadcrumbs="[['label' => 'Vendor', 'url' => route('admin.vendor.dashboard')], ['label' => __('admin.vendor.rooms.title'), 'url' => route('admin.vendor.rooms.index')], ['label' => __('admin.vendor.rooms.form.create')]]" />
    <x-alert />
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.vendor.rooms.store') }}" method="POST">
                @csrf
                <div class="mb-3">
                    <label class="form-label">{{ __('admin.vendor.rooms.form.hotel') }} *</label>
                    <select name="hotel_id" class="form-select @error('hotel_id') is-invalid @enderror" required>
                        <option value="">{{ __('admin.vendor.rooms.form.select_hotel') }}</option>
                        @foreach($hotels as $h)
                ...
                <div class="mb-3">
                    ...
                    <select name="cancellation_policy_preset" id="cancellation_policy_preset" class="form-select">
                        <option value="none" {{ old('cancellation_policy_preset', 'none') === 'none' ? 'selected' : '' }}>{{ __('admin.vendor.rooms.form.use_hotel_default') }}</option>
                ...
                <button type="submit" class="btn btn-primary">{{ __('admin.vendor.rooms.form.create') }}</button>
