...
4. **VNPay** calls IPN `GET /api/v1/payments/vnpay-ipn`:
   verify checksum → find payment by `vnp_TxnRef` → amount check
   (`vnp_Amount == amount x 100`) → idempotency via `webhook_events`
   (`provider=vnpay`, unique `event_id`) → dispatch `ProcessVnpayIpn` job.
