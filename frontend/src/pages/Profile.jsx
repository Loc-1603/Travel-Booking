...
export default function Profile() {
  ...
  const handleEditSubmit = (e) => {
    ...
    if (editPassword.trim() && editPassword !== editPasswordConfirm) {
      setEditErrors({ password: [t('auth.errors.passwordMismatch')] });
      return;
    }
    ...
    return (
      <div className="py-12 sm:py-16">
        <div className="rounded-2xl border border-stone-200 bg-white p-8 sm:p-12 text-center max-w-md mx-auto">
          ...
          <h2 className="text-xl font-semibold text-stone-900 mb-2">{t('profile.guests.signInToView')}</h2>
          <p className="text-stone-600 mb-6">{t('profile.guests.signInToManage')}</p>
          <Link
            to="/login"
    ...
    <div className="py-6 sm:py-8 lg:py-10">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8 sm:mb-10">
          <h1 className="text-2xl sm:text-3xl font-bold text-stone-900 tracking-tight">{t('profile.title')}</h1>
          <p className="text-stone-600 mt-1">{t('profile.subtitle')}</p>
        </div>
        ...
        <section className="rounded-2xl border border-stone-200 bg-white shadow-sm overflow-hidden">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-lg font-semibold text-stone-900">{t('profile.myBookings.title')}</h2>
              <p className="text-sm text-stone-600">{t('profile.myBookings.subtitle')}</p>
            </div>
            <span className="text-amber-600 font-medium text-sm">{t('common.view')} →</span>
          </Link>
        </section>
      </div>
                ...
                <div className="w-full">
                  ...
                  {user?.avatar_url && (
                    <label className="mt-2 flex items-center gap-2 text-sm text-stone-600 cursor-pointer">
                      ...
                      />
                      {t('profile.avatar.removeCurrent')}
                    </label>
                  )}
                  {editErrors.avatar && (
