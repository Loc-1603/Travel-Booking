...
export default function Checkout() {
  ...
  const handlePayClick = async () => {
    ...
    try {
      if (!user) {
        throw new Error(t('hotelDetail.booking.loginRequired'));
      }
      ...
      setPaymentError(err?.response?.data?.message || err?.message || t('checkout.errors.couldNotStartPayment'));
    }
  };
  ...
  if (!uuid) {
    return (
      <div className="py-6">
        <ErrorMessage message={t('checkout.errors.missingReference')} />
      </div>
    );
  }
  ...
  if (!booking && !user && !stateBooking) {
    return (
      <div className="py-6">
        <p className="text-[#5c5852] mb-2">{t('checkout.errors.sessionExpired')}</p>
        <p className="text-[#5c5852] mb-4">{t('checkout.errors.useEmailLink2')}</p>
        <Link to="/hotels" className="text-[#b8860b] underline hover:text-[#996f09]">{t('checkout.errors.searchHotels')}</Link>
      </div>
    );
  }
  ...
  if (!stateBooking && !booking && isError) {
    return (
      <div className="py-6">
        <ErrorMessage message={error?.response?.data?.message || error?.message || t('checkout.errors.bookingNotFound')} onRetry={() => refetch()} />
      </div>
    );
  }
  ...
  if (showSuccess) {
    return (
      <div className="py-12 max-w-lg mx-auto text-center">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-[#1a1a1a] mb-2">{t('checkout.success.title')}</h1>
        <p className="text-[#5c5852] mb-4">{t('checkout.success.confirmationSent')}</p>
        <div className="p-4 rounded-xl bg-[#f9edd1]/60 border border-[#e5c261]/60 mb-6">
          ...
          <p className="text-sm text-[#5c5852] mt-1">{t('checkout.success.bookingReference')}</p>
        </div>
  ...
  return (
    <div className="py-6">
      <h1 className="text-2xl sm:text-3xl font-bold text-[#1a1a1a] mb-6">{t('checkout.title')}</h1>

      <div className="flex flex-col lg:flex-row gap-8">
        {/* Left: Booking summary */}
        <div className="flex-1">
          <div className="rounded-2xl border border-[#e8e4dd] bg-white shadow-[0_4px_12px_rgb(26_26_26_/0.06)] p-6">
            <h2 className="font-semibold text-[#1a1a1a] mb-4">{t('checkout.summary.title')}</h2>
              ...
              <div className="border-t border-[#e8e4dd] pt-4 space-y-2">
                {booking.subtotal != null && (
                  <div className="flex justify-between text-sm">
                    <span className="text-[#5c5852]">{t('checkout.summary.subtotal')}</span>
                ...
                {(booking.late_checkout_amount ?? 0) > 0 && (
                  <div className="flex justify-between text-sm">
                    <span className="text-[#5c5852]">{t('checkout.summary.lateCheckout')}</span>
                    <span>{formatPrice(booking.late_checkout_amount, booking.currency)}</span>
                  </div>
                )}
                {(booking.tax_amount ?? 0) > 0 && (
                  <div className="flex justify-between text-sm">
                    <span className="text-[#5c5852]">{booking.hotel?.tax_name || t('checkout.summary.tax')}</span>
      ...
      >
        ← {t('checkout.payment.backTo', { target: user ? t('checkout.payment.profile') : t('checkout.payment.home') })}
      </Link>
    </div>
  );
}