    ...
    return (
      <div className="py-12 sm:py-16">
        <div className="rounded-2xl border border-[#e8e4dd] bg-white shadow-[0_4px_12px_rgb(26_26_26_/0.06)] p-8 sm:p-12 text-center max-w-md mx-auto">
          ...
          <h2 className="font-serif text-xl font-semibold text-[#1a1a1a] mb-2">{t('wishlist.guests.signInToView')}</h2>
          <p className="text-[#5c5852] mb-6">{t('wishlist.guests.signInToSave')}</p>
          <Link
            to="/login"
    ...
    return (
      <div className="py-6 sm:py-8 lg:py-10">
        <div className="max-w-6xl mx-auto">
          <div className="mb-8">
            <h1 className="font-serif text-2xl sm:text-3xl font-semibold text-[#1a1a1a] tracking-tight">{t('wishlist.title')}</h1>
            <p className="text-[#5c5852] mt-1">{t('wishlist.subtitle', { count: 0 })}</p>
          </div>
  ...
  if (isError) {
    return (
      <div className="py-6 sm:py-8">
        <div className="max-w-6xl mx-auto">
          <h1 className="font-serif text-2xl sm:text-3xl font-semibold text-[#1a1a1a] mb-6">{t('wishlist.title')}</h1>
          <div className="rounded-2xl border border-red-200 bg-red-50/50 p-6">
            <ErrorMessage
              message={error?.response?.data?.message || error?.message || t('wishlist.errors.couldNotLoad')}
              onRetry={() => refetch()}
            />
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          ...
          {items.length > 0 && (
            ...
            >
              ...
              {t('wishlist.actions.discoverMore')}
            </Link>
          )}
        </div>
        ...
        {items.length === 0 ? (
          <div className="rounded-2xl border border-[#e8e4dd] bg-white shadow-[0_4px_12px_rgb(26_26_26_/0.06)] p-12 sm:p-16 text-center">
            ...
            <h3 className="font-serif text-lg font-semibold text-[#1a1a1a] mb-2">{t('wishlist.empty.title')}</h3>
            <p className="text-[#5c5852] mb-8 max-w-sm mx-auto">{t('wishlist.empty.description')}</p>
            <Link
              to="/hotels"
                ...
                <HotelCard
                  ...
                  imageOverlay={
                    <button
                      ...
                      aria-label={t('wishlist.actions.remove')}
