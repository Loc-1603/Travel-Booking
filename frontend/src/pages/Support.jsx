...
function SupportContent() {
  ...
  if (isLoading) {
    return (
      <div className="w-full py-6 sm:py-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-stone-900 mb-2">{t('support.title')}</h1>
        <p className="text-stone-600 mb-8">{t('support.subtitle')}</p>
  ...
  if (isError) {
    return (
      <div className="w-full py-6 sm:py-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-stone-900 mb-6">{t('support.title')}</h1>
        <ErrorMessage message={error?.response?.data?.message || error?.message || t('support.errors.couldNotLoad')} onRetry={() => refetch()} />
      </div>
    );
  }
  ...
  return (
    <div className="w-full py-6 sm:py-8">
      <div className="flex flex-row sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-stone-900">{t('support.title')}</h1>
          <p className="text-stone-600 mt-1 max-w-2xl">{t('support.subtitle')}</p>
        </div>
        <Link
          to="/support/new"
      ...
      {tickets.length === 0 ? (
        <div className="rounded-2xl border border-stone-200/80 bg-white shadow-sm p-12 sm:p-16 text-center">
          ...
          <h3 className="text-lg font-semibold text-stone-900 mb-2">{t('support.empty.title')}</h3>
          <p className="text-stone-600 mb-8 max-w-sm mx-auto">{t('support.empty.description')}</p>
          <Link
            to="/support/new"
              ...
              >
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  ...
                  <span className="px-2.5 py-0.5 rounded-lg bg-amber-100 text-amber-800 text-xs font-medium">
                    {t(CATEGORY_LABELS[t.category] || t.category_label || t.category)}
                  </span>
      ...
      {meta.last_page > 1 && (
        <p className="text-sm text-stone-500 mt-6">
          {t('support.pagination.showing', { current: meta.current_page, total: meta.last_page, count: meta.total })}
        </p>
      )}
    </div>
  );
    ...
    return (
      <div className="py-12 sm:py-16">
        <div className="rounded-2xl border border-stone-200/80 bg-white shadow-sm p-8 sm:p-12 text-center max-w-md mx-auto">
          ...
          <h2 className="text-xl font-semibold text-stone-900 mb-2">{t('support.guests.signInToView')}</h2>
          <p className="text-stone-600 mb-6">{t('support.guests.signInToManage')}</p>
          <Link
            to="/login"
