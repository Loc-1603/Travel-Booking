...
const REVIEW_SCORE_OPTIONS = [
  ...
  { label: 'hotels.reviewScore.veryGood', value: 4 },
  { label: 'hotels.reviewScore.good', value: 3 },
  { label: 'hotels.reviewScore.pleasant', value: 2 },
];

const SORT_OPTIONS = [
  { value: '', label: 'hotels.sort.recommended' },
  { value: 'price_low', label: 'hotels.sort.priceLow' },
  { value: 'price_high', label: 'hotels.sort.priceHigh' },
  { value: 'rating', label: 'hotels.sort.rating' },
  { value: 'name', label: 'hotels.sort.name' },
  { value: 'distance', label: 'hotels.sort.distance', disabled: true },
];
...
function WishlistHeart({ hotelId, checkIn, checkOut, className = '' }) {
  ...
  if (!user) {
    return (
      <Link
        ...
        aria-label={t('hotels.wishlist.loginToSave')}
      >
  ...
  return (
    <button
      ...
      aria-label={inList ? t('hotels.wishlist.remove') : t('hotels.wishlist.add')}
    >
...
function HotelList() {
  ...
  const resultsHeadline = useMemo(() => {
    const n = total;
    const unit = n === 1 ? t('common.hotel') : t('common.hotels');
    if (latitude && longitude && radiusKm) {
      return { line1: `${n} ${unit} ${t('hotels.results.withinRadius', { radius: radiusKm })}`, line2: t('hotels.results.mapAreaFilters') };
    }
    if (city) {
      return { line1: `${city}: ${n} ${unit}`, line2: t('hotels.results.totalsUpdate') };
    }
    if (country && !city) {
  ...
  if (isError) {
    return (
      <div className="py-6">
        <ErrorMessage message={error?.response?.data?.message || error?.message || t('errors.unknownError')} onRetry={() => refetch()} />
      </div>
    );
  }
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
            ...
            <div className="flex items-center gap-2">
              <label htmlFor="sort" className="text-sm text-[#5c5852]">{t('hotels.filters.sortBy')}</label>
              <select
                id="sort"
                value={sort}
              ...
              >
                {SORT_OPTIONS.map((opt) => (
                  <option key={opt.value} value={opt.value} disabled={opt.disabled}>
                    {t(opt.label)}{opt.disabled ? ` ${t('hotels.sort.comingSoon')}` : ''}
                  </option>
                ))}
              </select>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="py-16 text-center" role="status">
              <p className="text-[#5c5852] text-lg font-medium mb-2">{t('hotels.noResults')}</p>
              <p className="text-[#5c5852] mb-6">{t('hotels.noResultsDesc')}</p>
              <Link
                to="/"
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-4 sm:gap-6">
                {hotels.map((h) => (
                  ...
                  >
                    <span className="mt-2 inline-flex items-center gap-1.5 text-sm font-medium text-[#b8860b] group-hover:text-[#996f09]">
