...
export function HotelCard({ hotel, nights, dealLabel, originalPrice, to, currency = 'VND', imageOverlay, children }) {
  ...
  const content = (
    <>
      <div className="w-full shrink-0 aspect-[4/3] bg-[#e8e4dd] relative overflow-hidden flex-shrink-0">
        {img?.url ? (
          <img
            src={img.url}
  ...
  if (href.startsWith('http') || !href) {
    return (
      <div className="group flex! flex-col! items-stretch! justify-start! text-left p-0! rounded-2xl overflow-hidden border border-[#e8e4dd] bg-white shadow-[0_4px_12px_rgb(26_26_26_/0.06)] hover:shadow-[0_12px_28px_rgb(26_26_26_/0.1)] hover:border-[#d4cec4] transition-all duration-300">
        {content}
      </div>
    );
  }
  ...
  return (
    <Link
      to={href}
      className="group flex! flex-col! items-stretch! justify-start! text-left p-0! rounded-2xl overflow-hidden border border-[#e8e4dd] bg-white shadow-[0_4px_12px_rgb(26_26_26_/0.06)] hover:shadow-[0_12px_28px_rgb(26_26_26_/0.1)] hover:border-[#d4cec4] transition-all duration-300"
    >
      {content}
    </Link>
  );
